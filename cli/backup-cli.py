...
if __name__ == "__main__":
    ...
    def graph_stats(self):
        ...
        try:
            ...
            print(f"  Total Nodes: {sum(types.values())}")
            print(f"  Total Edges: {len(kg.get_edges_by_type('')) if hasattr(kg, 'get_all_edges') else 'N/A'}")
            print(f"\n  By Type:")
    ...
    def search_graph(self, query: str):
        ...
        try:
            ...
            if results:
                print(f"\n🔍 Results for: {query}")
                for r in results[:10]:
                    print(f"  • {r.get('name', r.get('id', 'unknown'))}")
            else:
                print(f"No results for: {query}")
        except Exception as e:
